...
class StatisticsController extends Controller
{
    public function fetchStatistics(Request $request)
    {
        ...
        // Apply date range condition only if both start_date and end_date are provided
        if ($request->start_date && $request->end_date) {
            $startDate = Carbon::parse($request->start_date)->startOfDay();  // Start from the beginning of the day
            $endDate = Carbon::parse($request->end_date)->endOfDay();        // End at the end of the day
            ...
            foreach ($request->filters as $filter) {
                ...
                switch ($key) {
                    ...
                    case 'age_category':
                        ...
                        if (count($ageRange) == 2) {
                            $query->whereBetween('age_category', [(int)$ageRange[0], (int)$ageRange[1]]);
                        }
                        break;
                    case 'language':
        ...
        // Get total users after applying filters (and optional date range)
        ...
        if ($request->start_date && $request->end_date) {
            $responseQuery->whereBetween('created_at', [$startDate, $endDate]);
        }

        $totalSubmissions = $responseQuery->select('user_id', DB::raw('COUNT(*) as total_submissions'))
            ->groupBy('user_id')
            ->get();
        ...
        if ($request->start_date && $request->end_date) {
            $downloadQuery->whereBetween('created_at', [$startDate, $endDate]);
        }
